...
      setLoading(true);
      setError("");
      console.log("📥 Bắt đầu tải NFT details TỪ BACKEND cho Token ID:", tokenId);

      // 1. GỌI API BACKEND (CHỈ 1 REQUEST)
      const response = await fetch(`http://localhost:5000/api/nft/detail/${tokenId}`);
      
      if (!response.ok) {
...
      return;
    }
    if (!contract || !signer) { // Phải kiểm tra cả signer
...
  };
  ...
  const isOwner =
    walletAddress && nftData && // Thêm kiểm tra nftData
